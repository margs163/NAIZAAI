...
def transcribe_audio_in_chunks(audio_path: Path, chunk_length: int = 600, overlap: int = 10) -> dict:
    ...
    try:
        ...
        print(f"duration{duration} // (chunks_ms(600.000) - overlap_ms(10)): {total_chunks}")
...
def extract_audio(path_raw: str, output_path: str) -> None:
    command = f"ffmpeg -i {path_raw} -ab 160k -ac 2 -ar 44100 -vn {output_path}"
...
if __name__ == "__main__":
    ...
    command = f"ffmpeg -y -i {path_raw} -ab 160k -ac 2 -ar 44100 -vn {output_path}"
    subprocess.call(command, shell=True)

    result = transcribe_audio_in_chunks(Path(output_path))

    for segment in result["segments"]:
        print("Start: ", segment["start"], "End: ", segment["end"], "Text: ", segment["text"])